...
| `external_free` | modules/units matching `from` must be attributed zero external packages — engaged by presence (a pure match passes for real), contamination fails naming module + packages, a pattern matching nothing present is vacuous; belongs on leaf layers, not composition roots | `from` |
...
| Go | the `go.mod` module path's packages — Go packages are **units** | member modules of a `go.work` workspace (native fact); otherwise the model carries none and the spec's `[[module]]` declarations supply the grouping (declared derivation) |
...
- **Module-granularity rules enforce only over assigned packages.** Laundered-edge hop ownership, submodule contracts, and module-edge checks see the derived tier — membership from the declarations, edges from real code — so they engage on claimed territory and nothing else.
