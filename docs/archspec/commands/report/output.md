...
- `--output <path>`: report written to the file, **stdout stays empty**, exit code `0`.
- `[output] report` in `archspec.toml`: report written to the configured path
  (relative to the project root), **stdout stays empty**, exit code `0`.
...
from that (`markdown`, `json`) is therefore printed to **stdout** and the
configured path stays untouched — serializing JSON into `report.md` would
...
3. **Violations by category** — the per-category violation counts, in a separate table that appears **only when the run reports at least one violation** (omitted from a clean report).
4. **Diff** — the full structural difference between the extracted and the declared model (ADR-010, `../../../archspec-design.md` §9).
5. **Summary line** — total violations by category, or a clean-result statement.

The Markdown format inserts, between the metrics and the diff:

1. **Diagram** — the current file-level import-map diagram as a Mermaid code fence (`## Diagram`). Every supported language has a file-level scanner, so the section carries a diagram on any tree.
2. **Forbidden imports** (`## Forbidden imports`) — every forbidden edge present, disallowed cross-component dependency, and facade dependency, each on its own line.
3. **Cycles** (`## Cycles`) — every cycle detected, each on its own line.
...
| edges (internal) | dependency edges within a component |
...
- `facade dependency` — an internal module depends on its own publication-only crate root (structural; no constraint declares it; emitted by the rust driver only — csharp/go trees populate no root-facade fact, so the rule never fires there)
- `contract leak` — exposed surface bleeds outside the declared contract; the submodule form needs the module tier (rust, csharp, and go — go acquires one from go.work members or from spec modules claiming its packages)
