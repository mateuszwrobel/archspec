...
- `--output <path>`: canonical model written to the file, **stdout stays empty**, exit code `0`. `--output -` prints the model JSON on stdout and creates no file.
...
12. **Roles** — a serialized map of model path → role, drawn from the closed vocabulary {`facade`, `composition`}, derived by each driver from its own facts (rust: root-defines-nothing and wiring-bin roots; csharp: using-only umbrella root and DI-registration entrypoint; go: the `package main` module, composition only). Consumers (`verify`, `report`, `inspect`) read the map instead of re-deriving roles. A module whose driver states no role carries no entry, and a tree with no derivable role serializes no `roles` key at all — absence states "no role stated", never "role denied" (derivation details in `acceptance.md`).
